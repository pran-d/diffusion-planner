...
def reconstruct_sbto_trajectory(
    ...
    # R = RotZ(yaw) @ R_no_yaw
    #
    # Each frame's delta_yaw is relative to ref_yaw and already wrapped to [-π, π].
    # Adding ref_yaw can put yaw outside [-π, π], so we normalize.
    # Then enforce temporal continuity: if adjacent yaw values differ by >π it is a
    # wrap-around artefact, not genuine spinning — correct by unwrapping.
    traj_yaw = normalize_angle(ref_yaw[:, None] + traj_delta_yaw.squeeze(-1))  # (B, T)
